...
thread number, address and domain.
\\
\par\noindent
...
\hline
\multicolumn{3}{|l|}{\textbf{thread number and address registers}} \\
\hline
\texttt{0x05} & num       & user thread number \\
\texttt{0x06} & addr      & user thread address \\
\texttt{0x07} & domain    & user thread domain \\
\hline
\end{tabularx}
...
this register is visible if \texttt{syscontrol.F} is enabled.
\\
\par\noindent
...
precision. this register is visible if \texttt{syscontrol.F} is enabled.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
this register is visible if \texttt{syscontrol.T} is enabled.
...
\subsection{thread number and address registers}
the section describes the user-level thread number and address registers.

\subsubsection{thread number (\texttt{num})}
\texttt{num} is a read-only register containing an alias of the
thread number for this thread from \texttt{snum}.
this register is visible if \texttt{scontrol.N} is enabled.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\small,anchor=base] at (16.25,0.25)  {\emph{thread number}};
...
\subsubsection{domain address (\texttt{domain})}
...
\texttt{0x24} & sidx      & system thread index \\
\texttt{0x25} & snum      & system thread number \\
\texttt{0x26} & saddr     & system thread address \\
\texttt{0x27} & sdomain   & system thread domain \\
\texttt{0x28} & starget   & system target address \\
\texttt{0x29} & smessage  & system message interrupt \\
...
\draw [black!95] (29,0)    rectangle (30,1);
\node [font=\tiny,anchor=base] at (29.5,1.25)  {\textsf{2}};
\node [font=\small,anchor=base] at (29.5,0.25)  {\texttt{N}};
\draw [black!95] (28,0)    rectangle (29,1);
\node [font=\tiny,anchor=base] at (28.5,1.25)  {\textsf{3}};
...
\verb|1 << 2| & N   & user thread number and address enabled \\
...
\verb|1 << 2| & N   & user thread number and address feature \\
...
\newpage
\subsubsection{system thread index (\texttt{sidx})}
\texttt{sidx} is a read-write register containing the thread index for
this thread. the thread index is a system-wide monotonically increasing
number used for indexing of threads by the operating system. this number
is uniquely assigned during system initialization.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\small,anchor=base] at (16.25,0.25)  {\emph{thread index}};
\node [font=\tiny,anchor=base] at (0.25,1.25)  {\textsf{63}};
\end{tikzpicture}

\subsubsection{system thread number (\texttt{snum})}
\texttt{snum} is a read-write register containing the thread number for
this thread. the thread number is used for user-level thread indexing
and is assigned by the operating system. it is up to the operating system
to ensure it does not alias within user-level processes.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
zero in the \emph{A} field is a \emph{thread address}.
...
\subsubsection{thread domain (\texttt{domaddr}$[\mathit{ndom}]$)}
...
\subsubsection{memory memory registers}
