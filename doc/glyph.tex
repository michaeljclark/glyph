...
\hline
\multicolumn{3}{|l|}{\textbf{privileged machine registers}} \\
\hline
\texttt{0x42} & mtlbkey   & machine translation lookaside buffer key \\
\texttt{0x43} & mtlbent   & machine translation lookaside buffer entry \\
\hline
\end{tabularx}
...
\verb|13| & page-fault-miss    & page translation miss \\
\verb|14| & capability-fault   & capability fault \\
\verb|15| & domain-fault       & domain fault \\
\hline
...
\draw [black!95] (26,0)    rectangle (27,1);
\node [font=\tiny,anchor=base] at (26.5,1.25)  {\textsf{5}};
\node [font=\small,anchor=base] at (26.5,0.25)  {\texttt{P}};
\draw [black!95] (25,0)    rectangle (26,1);
\node [font=\tiny,anchor=base] at (25.5,1.25)  {\textsf{6}};
\node [font=\small,anchor=base] at (25.5,0.25)  {\texttt{C}};
\draw [black!95] (24,0)    rectangle (25,1);
\node [font=\tiny,anchor=base] at (24.5,1.25)  {\textsf{7}};
\node [font=\small,anchor=base] at (24.5,0.25)  {\texttt{V}};
\end{tikzpicture}
...
\verb|1 << 5| & P   & physical permissions check enabled \\
\verb|1 << 6| & C   & capabilities check enabled \\
\verb|1 << 7| & V   & virtual domains enabled \\
\hline
\end{tabular}
...
\verb|1 << 5| & P   & physical permissions check feature \\
\verb|1 << 6| & C   & capabilities check feature \\
\verb|1 << 7| & V   & virtual domains feature \\
\hline
\end{tabular}
...
\newpage
\subsection{privileged machine registers}
the section describes the privileged machine registers for address
translation. the registers in this section enable a \emph{page-fault-miss}
...
\texttt{mtlbkey} is a read-only register containing the translation
virtual address for the current page translation miss fault.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\texttt{mtlbent} is a read-write register containing the machine translation
lookaside buffer entry for the current page translation miss fault. this
register is populated by the page translation miss fault handler with
\emph{ASID}, a physical page number, a page color, plus several permission
and metadata bits in the same format as the metadata bits defined for page
table entries, with the exception that the accessed and dirty bits are
replaced with the page size. table~\ref{tab:64-bit-page-table-entry-fields}
in the address translation section describes the metadata bits.
\\
\par\noindent
\emph{ASID 0} is a valid address space for tagged caches. the \emph{G}
bit is used to indicate that the \emph{ASID} field is to be ignored.
the physical page number points to the translated physical page. it is
illegal for the \emph{read, write} and \emph{exec} permissions to all be
zero and it is illegal for \emph{write} to be set without \emph{read}.
this register is initialized to zero on address translation misses.
...
\node [font=\tiny,anchor=base] at (0.5,1.25)  {\textsf{63}};
\node [font=\tiny,anchor=base] at (7.5,1.25)  {\textsf{48}};
\node [font=\small,anchor=base] at (4.0,0.25)  {\emph{ASID}};
\end{tikzpicture}
...
\hline
\verb|0| & $2^{12}$ & 4KiB page \\
\verb|1| & $2^{21}$ & 2MiB page \\
\verb|2| & $2^{30}$ & 1GiB page \\
\hline
\end{tabular}
\caption{64-bit machine translation lookaside buffer entry size field}
\label{tab:64-bit-machine-translation-lookaside-buffer-entry-size-field}
\end{table}
...
includes capability registers controlling color permissions enabled
by \emph{page table color} bits in page table entries and capabilities
that restrict access to system, domain, and capabilities registers
and instructions.
\\
\par\noindent
capability registers can be read and written using the \texttt{sysread},
\texttt{syswrite}, \texttt{sysset}, and \texttt{sysclear} instructions.
the presence of capability registers is indicated by \texttt{sfeature.C}
and they are available when \texttt{scontrol.C} is enabled.
\\
\par\noindent
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\multicolumn{3}{|l|}{\textbf{color permission registers}} \\
\hline
\texttt{0x50} & colorread      & color read permissions \\
\texttt{0x51} & colorwrite     & color write permissions \\
\texttt{0x52} & colorexec      & color exec permissions \\
\hline
\multicolumn{3}{|l|}{\textbf{color capability registers}} \\
\hline
\texttt{0x53} & colorsys       & color system capabilities \\
\texttt{0x54} & colordom       & color domain capabilities \\
\texttt{0x55} & colorcap       & color capability capabilities \\
\hline
\multicolumn{3}{|l|}{\textbf{color matrix registers}} \\
\hline
...
\newpage
\subsection{color permission registers}

this section describes the \emph{color permission registers}.
the \emph{color permission registers} are capability registers that
protect access to memory based on \emph{page table colors}. when the
capabilities extension is enabled the \emph{color permission registers}
are combined with page table permissions to limit maximum \emph{read},
\emph{write}, or \emph{execute} permission based on page table colors.

\subsubsection{color read permissions (\texttt{colorread})}
...
that are used to check loads from pages that have page table entries with
matching page table colors.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\subsubsection{color write permissions (\texttt{colorwrite})}
...
that are used to check stores to pages that have page table entries with
matching page table colors.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
\subsubsection{color exec permissions (\texttt{colorexec})}
...
that are used to check execution in pages that have page table entries with
matching page table colors.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
\newpage
\subsection{color capability registers}

this section describes the \emph{color capability registers}.
the \emph{color capability registers} are capability registers that
describe access to privileged system registers and instructions
based on \emph{page table colors} and register or instruction category.
when the capabilities extension is enabled the \emph{color capability
registers} limit access to privileged system registers and instructions
based on the system register categories: \emph{system}, \emph{domain},
and \emph{capability}.

\subsubsection{color system capabilities (\texttt{colorsys})}
\texttt{colorsys} is a read-write register containing an array
of \textbf{\emph{ncol}} bits containing system capabilities for \emph{color-x}
that are used to check access to privileged trap registers,
privileged system registers and privileged debug registers
from executable pages that have page table entries with a matching
page table color.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
\subsubsection{color domain capabilities (\texttt{colordom})}
\texttt{colordom} is a read-write register containing an array
of \textbf{\emph{ncol}} bits containing domain capabilities for \emph{color-x}
that are used to check access to privileged domain registers from
executable pages that have page table entries with a matching
page table color. this register is present if both the domains and
capabilites features are present.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\subsubsection{color capability capabilities (\texttt{colorcap})}
\texttt{colorcap} is a read-write register containing an array
of \textbf{\emph{ncol}} bits containing capability capabilities for \emph{color-x}
...
\newpage
\subsection{color matrix registers}

this section describes the color matrix registers. the \emph{color
...
page table colors. when the capabilities extension is enabled the color
matrix registers limit \emph{read}, \emph{write}, or \emph{execute}
permission based on executable pages that have page table entries with
source \emph{page table color} in memory accesses or branches to memory
addresses in pages with matching \emph{page table color} index.
...
permissions that are used to check loads from executable pages that have
page table entries with a matching register number \emph{page table color}
to memory addresses in memory pages that have page table entries with a
matching \emph{page table color} array index.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
permissions that are used to check stores from executable pages that have
page table entries with a matching register number \emph{page table color}
to memory addresses in memory pages that have page table entries with a
matching \emph{page table color} array index.
\\
...
permissions that are used to check branches from executable pages that have
page table entries with a matching register number \emph{page table color}
to branch addresses in executable pages that have page table entries with a
matching \emph{page table color} array index.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
the architecture provides an optional virtual domain~\cite{barham2003xen}
extension that includes registers for domain address translation, domain
routing and context switching for system traps, domain traps, and
capability traps. capability trap routing requires both the virtual
domains and capabities features to be present and enabled.
\\
\par\noindent
domain registers can be read and written using the \texttt{sysread},
\texttt{syswrite}, \texttt{sysset}, and \texttt{sysclear} instructions.
the presence of domain registers is indicated by \texttt{sfeature.V}
and they are available when \texttt{scontrol.V} is enabled.
\\
\par\noindent
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\multicolumn{3}{|l|}{\textbf{domain control registers}} \\
\hline
\texttt{0x50} & domlast  & domain number -- last \\
\texttt{0x51} & domcurr  & domain number -- current \\
\texttt{0x52} & domnext  & domain number -- next \\
\texttt{0x1000...0x100f} & dommatrix$[n]$ & domain matrix \\
\hline
\multicolumn{3}{|l|}{\textbf{domain routing registers}} \\
\hline
\texttt{0x2000...0x200f} & domsys$[n]$    & domain system default domain vector \\
\texttt{0x3000...0x300f} & domdom$[n]$    & domain domain default domain vector \\
\texttt{0x4000...0x400f} & domcap$[n]$    & domain capability default domain vector \\
\hline
\multicolumn{3}{|l|}{\textbf{domain context registers}} \\
\hline
\texttt{0x5000...0x500f} & domaddr$[n]$   & domain address vector \\
\texttt{0x6000...0x600f} & domptr$[n]$    & domain page table root vector \\
\texttt{0x7000...0x700f} & domthpc$[n]$   & domain trap handler program counter vector  \\
\texttt{0x8000...0x800f} & domthib$[n]$   & domain trap handler immediate base vector \\
\hline
\end{tabularx}
...
access to last and currnt domain number and control the next domain.
...
registers containing a matrix of the domains that each domain is allowed
to transition to as the next domain.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
for context switching of system traps, domain traps and capability traps.

\subsubsection{domain system default domain vector (\texttt{domsys}$[\mathit{ndom}]$)}
...
\subsubsection{domain domain default domain vector (\texttt{domdom}$[\mathit{ndom}]$)}
...
extension is enabled in \texttt{scontrol.V}.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\subsubsection{domain capability default domain vector (\texttt{domcap}$[\mathit{ndom}]$)}
...
\texttt{scontrol.C}. this register is present if both the domains and
capabilites features are present.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
\subsubsection{domain address vector (\texttt{domaddr}$[\mathit{ndom}]$)}
...
\subsubsection{domain page table root vector (\texttt{domptr}$[\mathit{ndom}]$)}
...
\subsubsection{domain trap handler program counter vector (\texttt{domthpc}$[\mathit{ndom}]$)}
...
\subsubsection{domain trap handler immediate base vector (\texttt{domthib}$[\mathit{ndom}]$)}
...
and metadata bits. the physical page number either points to the next page
...
\\
\par\noindent
if the physical permissions feature \emph{sfeature.P} is present and
\emph{scontrol.P} is enabled, then physical pages must be present in the
...
\Function{find\_page}{$\mathit{type},\mathit{level},\mathit{ppn},\mathit{ta}$}
  ...
  \If {$\mathit{sfeature.P} \land \mathit{scontrol.P} \land \mathit{type} =
      \mathit{virtual}$}
