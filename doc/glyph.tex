...
\hline
\texttt{0x42} & mtlbent   & machine translation lookaside buffer entry \\
\hline
\end{tabularx}
...
lookaside buffer entry for the current page translation miss fault.
the machine translation lookaside buffer entry register contains \emph{ASID},
...
with the exception that the accessed and dirty bits are replaced with the
page size. table~\ref{tab:64-bit-page-table-entry-fields} in the address
...
the \texttt{tvalue} register contains the translation address for the
current page translation miss fault.
