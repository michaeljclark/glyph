...
as its dominant principle, along with several features not yet present
...
the proposed architecture, followed by chapters on constant streams, the
...
counter} called \emph{pc-relative} addresses which are stored inside
instruction immediate values or stored indirectly inside constants
accessed using \emph{ib-relative} addresses. section~\ref{sec:constant-stream}
...
constant memory is a memory region dedicated to constants. it is
organized as a word-addressable store like main-memory, but is read-only
...
instruction register slots, are sized, scaled and aligned addresses computed
...
\item \texttt{movib} \emph{move-immediate-block} uses an unsigned displacement
to access constants.
...
\emph{ib-relative} addresses stored inside instruction register slots.
\\
\par\noindent
...
addresses stored inside instruction register slots, which are sized, scaled,
...
\emph{5} \textrightarrow \emph{greater or equal (unsigned)},
\emph{6} \textrightarrow \emph{conditional move}, or
\emph{7} \textrightarrow \emph{negated conditional move}.
\begin{Verbatim}
    match fun3
