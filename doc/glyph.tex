...
\\
\par\noindent
this table gives an overview of the control and status registers.
...
\texttt{0x21} & sptr      & system page table root \\
\texttt{0x22} & saddr     & system thread address \\
\texttt{0x23} & starget   & system target address \\
\texttt{0x24} & smessage  & system message interrupt \\
\texttt{0x25} & stimer    & system deadline timer \\
...
\texttt{0x31} & dcycle     & debug cycle counter \\
\texttt{0x32} & dinst      & debug instruction counter \\
\texttt{0x33} & dstop      & debug stop instruction \\
\texttt{0x34} & dfetch     & monitored fetch address \\
\texttt{0x35} & dread      & monitored read address \\
\texttt{0x36} & dwrite     & monitored write address \\
\hline
\end{tabular}
...
\texttt{scontrol} is a read-write register containing control bits for
address translation. the \texttt{A} field controls whether the user pages
and be read or written by the supervisor. the \texttt{E} field controls
...
\emph{root} field contains the physical address of the page table
root structure.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
\node [font=\small,anchor=base] at (15,0.25)  {\emph{root}};
...
\texttt{saddr} is a read-only register containing the system-wide address
for this hardware thread.

\subsubsection{system target (\texttt{starget})}
\texttt{starget} is a read-write register containing the system-wide
address for a hardware thread that is the target of edge-triggered virtual
interrupts. a target address of all-zeros is the boot service processor.
a target address of all-ones is the broadcast address for all threads.

\subsubsection{system message (\texttt{smessage})}
\texttt{smessage} is a write-only register to cause an edge-triggered
...
interrupt is delivered when next enabled.
...
register, a \emph{timer interrupt} is triggered. if interrupts are not
enabled on this hardware thread, the timer interrupt is delivered when
next enabled.
...
\hline
\textbf{Levels} & \textbf{VPN\tablefootnote{Virtual Page Number} bits} &
\textbf{Page bits} & \textbf{Virt bits} & \textbf{Phys bits} & \textbf{Entry bits} &
...
\label{fig:8-byte-pte-structure}
\end{figure}
...
\hline
\end{tabular}
\caption{page table entry fields}
\label{tab:page-table-entry-fields}
\end{table}

\newpage
\subsection{page table walker}
the page table walker performs page table lookups to translate 64-bit
virtual addresses to 64-bit physical addresses. there are 48 active bits
in virtual addresses and the remaining bits are sign-extended. physical
addresses have 56-bits and the upper 8-bits are ignored.
...
\node [font=\tiny,anchor=base] at (25.5,1.25)  {\textsf{12}};
\node [font=\small,anchor=base] at (23.75,0.25)  {\emph{vpn[3]}};
...
\node [font=\small,anchor=base] at (19.25,0.25)  {\emph{vpn[2]}};
...
\node [font=\small,anchor=base] at (14.75,0.25)  {\emph{vpn[1]}};
...
\node [font=\small,anchor=base] at (10.25,0.25)  {\emph{vpn[0]}};
...
\par\noindent
the page table address translation process is as follows:
...
\begin{algorithmic}[1]
 ...
 \Function{translate}{$va$}
  ...
  \EndIf
  \State $\mathit{ppn} = \mathit{sptr.root} << \mathit{page\_bits}$
  \Comment{fetch the page table root pointer}
  \For{$\mathit{level} = 1$ to $\mathit{num\_levels}$}
      \State $\mathit{shift} = (\mathit{num\_levels} - \mathit{level}) \times
        \mathit{vpn\_bits} + \mathit{page\_bits}$
      \Comment{calculate virtual page number}
      \State $\mathit{vpn} = (\mathit{va} >> \mathit{shift}) \land
        ((1 << \mathit{vpn\_bits}) - 1)$
      \State $\mathit{pte} = \Call{load\_64}{\mathit{ppn} +
        \mathit{vpn} * \mathit{pte\_size}}$
      \Comment{load PTE}
      \If {$\lnot\mathit{pte.V}$}
        \Comment{check valid}
        \State \textbf{raise} \textbf{fault}
      \ElsIf {$\lnot(\mathit{pte.R} \lor \mathit{pte.W} \lor \mathit{pte.X})$}
      \Comment{no read, write or exec is a pointer}
        \State $\mathit{ppn} = \mathit{pte.ppn}$
        \State \textbf{continue}
      \ElsIf{$\mathit{pte.W} \land \lnot\mathit{pte.R}$}
        \Comment{check write must have read}
        \State \textbf{raise} \textbf{fault}
      \ElsIf {$(\mathit{pte.R} \land \lnot\mathit{pte.A}) \lor
          (\mathit{pte.W} \land \lnot\mathit{pte.D})$}
        \Comment{check accessed or dirty}
        \State \textbf{raise} \textbf{fault}
      \ElsIf {$\mathit{mode} = S \land \lnot\mathit{scontrol.E} \land
          \mathit{pte.U} \land (\mathit{pte.X})$}
        \Comment{check execute user page}
        \State \textbf{raise} \textbf{fault}
      \ElsIf {$\mathit{mode} = S \land \lnot\mathit{scontrol.A} \land
          \mathit{pte.U} \land (\mathit{pte.R} \lor \mathit{pte.W})$}
        \Comment{check access user page}
        \State \textbf{raise} \textbf{fault}
      \ElsIf {$\mathit{pte.ppn} \land ((1 << (\mathit{shift} - \mathit{page\_bits})) - 1)$}
        \Comment{check superpage is aligned}
        \State \textbf{raise} \textbf{fault}
      \EndIf
      \State \Return $(\mathit{ppn} << \mathit{page\_bits}) + (\mathit{va} \land
      ((1 << (\mathit{page\_bits} + \mathit{level} \times \mathit{vpn\_bits})) - 1))$
  \EndFor
 \EndFunction
