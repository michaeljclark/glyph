...
\texttt{i8x\nb}   & \texttt{\nb{}b}  & \emph{\nb * byte}   & \nb x 8-bit signed integer (byte) \\
\texttt{i16x\nw}  & \texttt{\nw{}w}  & \emph{\nw * word}   & \nw x 16-bit signed integer (word) \\
\texttt{i32x\nd}  & \texttt{\nd{}d}  & \emph{\nd * dword}  & \nd x 32-bit signed integer (double-word) \\
\texttt{i64x\nq}  & \texttt{\nq{}q}  & \emph{\nq * qword}  & \nq x 64-bit signed integer (quad-word) \\
\texttt{i128x\no} & \texttt{\no{}o}  & \emph{\no * oword}  & \no x 128-bit signed integer (octa-word) \\
\texttt{u8x\nb}   & \texttt{\nb{}bu} & \emph{\nb * ubyte}  & \nb x 8-bit unsigned integer (byte) \\
\texttt{u16x\nw}  & \texttt{\nw{}wu} & \emph{\nw * uword}  & \nw x 16-bit unsigned integer (word) \\
\texttt{u32x\nd}  & \texttt{\nd{}du} & \emph{\nd * udword} & \nd x 32-bit unsigned integer (double-word) \\
\texttt{u64x\nq}  & \texttt{\nq{}qu} & \emph{\nq * uqword} & \nq x 64-bit unsigned integer (quad-word) \\
\texttt{u128x\no} & \texttt{\no{}ou} & \emph{\no * uoword} & \no x 128-bit unsigned integer (octa-word) \\
\texttt{f16x\nw}  & \texttt{\nw{}hf} & \emph{\nw * half}   & \nw x IEEE 754 16-bit floating-point (half-precision) \\
\texttt{f32x\nd}  & \texttt{\nd{}sf} & \emph{\nd * float}  & \nd x IEEE 754 32-bit floating-point (single-precision) \\
\texttt{f64x\nq}  & \texttt{\nq{}df} & \emph{\nq * double} & \nq x IEEE 754 64-bit floating-point (double-precision) \\
\texttt{f128x\no} & \texttt{\no{}qf} & \emph{\no * quad}   & \no x IEEE 754 128-bit floating-point (double-precision) \\
\hline
\end{tabular}
