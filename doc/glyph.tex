...
resulting \emph{program counter} address is \emph{$[pc + simm9 \times 2 + 2]$}.
\begin{Verbatim}[baselinestretch=1.0,fontsize=\small]
    pc += simm9 * 2 + 2
\end{Verbatim}
...
\emph{program counter} address is \emph{$[pc + simm9 \times 2 + 2]$},
...
loads a 64-bit constant addressed by \emph{$[ib + uimm6 \times 8]$} containing
a \emph{i32x2} relative address vector, which it adds it to \emph{(pc,ib)},
then saves the relative address vector in the \emph{rc} register.
\begin{Verbatim}[baselinestretch=1.0,fontsize=\small]
...
loads a 64-bit constant addressed by \emph{$[ib + uimm6 \times 8]$}
containing a \emph{i32x2} relative address vector, then subtracts the
\emph{i32x2} relative address vector in the \emph{rc} register from it,
and adds the result to \emph{(pc,ib)}.
\begin{Verbatim}[baselinestretch=1.0,fontsize=\small]
    {rpc,rib} = (i32x2)r[rc]
...
constant addressed by \emph{$[ib + uimm6 \times 8]$} then saves it to the
\emph{rc} register.
\begin{Verbatim}[baselinestretch=1.0,fontsize=\small]
    r[rc] = (i64)[ib + uimm6 * 8]
\end{Verbatim}
