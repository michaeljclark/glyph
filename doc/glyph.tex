...
chain of relative address vectors that can be signed and encrypted to
form a verifiable chain of return addresses for control flow integrity.
this is due to the use of relative address vectors. the return strictly
requires a block entry delta, and \texttt{link} adds and subtracts to and
from the link register. signed and/or encrypted relative address vectors are
a possible extension to the \texttt{pin} and \texttt{link} instructions.
...
\needspace{8\baselineskip}
\subsection{link}
\insrcimmsix{fun3}{uimm}{00100}{00}{\textbf{link.i64}\: fun3, ib64(uimm6)}
\\
...
\begin{Verbatim}
    ...
    (cpc,cib) = m<i32x2>[ib + uimm6 * 8]
    (lpc,lib) = fun3 == jtlib ? r<i32x2>[lr] : 0
...
\ainsrcimmsix{4}{fun3}{uimm}{00100}{00}{\textbf{link.i64}\: fun3, ib64(uimm6)}
