...
instructions use a variable length format with a 16-bit packet
that supports 16, 32, 64, and 128-bit instruction words. the
instruction packet has been designed to use a \emph{super regular}
scheme, whereby successive instruction packets extend the fields
in the previous packet. instruction words can begin on any 16-bit
aligned address.
...
fixed position in every 16-bit instruction packet, to reduce the complexity
of size-decoding for variable length instructions. this table shows the
size fields for the different instruction sizes:
\\
\begin{table}[h]
\centering
\small
