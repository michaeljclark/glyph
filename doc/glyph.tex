...
instruction and constant memory are two sets of storage which are likely
to be \emph{SRAM}\footnote{\emph{SRAM}\,– Static random-access memory.}.
instruction and constant memory are organized as word-addressable stores
similar to main-memory, although they are read-only and restricted
to instructions and constants.
\\
\par\noindent
instruction memory is addressed using \emph{pc-relative} addresses in
instruction immediate values or indirectly via constants using
\emph{ib-relative} addresses in register slots.
\\
\par\noindent
constant memory is addressed using \emph{ib-relative} addresses in register
slots, which are typed, scaled and aligned addresses. the following diagrams
shows $ib8(n)$ through $ib64(n)$ \emph{ib-relative} addresses for access
to 8-bit through to 64-bit constants respectively: \emph{note: ib-relative
