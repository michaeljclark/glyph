...
VLEN & 512, 1024  & width of vector registers in bits \\
...
\emph{vector-max} & 64 & 64 & 64 & 64  & 128 & 1024 & 128 \\
\hline
\end{tabular}
...
\item 64-bit \emph{immediate base} register (aligned to 64 bytes).
\item 64 \texttimes\,128-bit scalar registers (\textsf{r0} --- \textsf{r63}).
\item 64 \texttimes\,1024-bit vector registers (\textsf{v0} --- \textsf{v63}).
\end{itemize}
...
\foreach \x in {1,...,7} {
  \draw[black!20] (2*\x,-1) -- (2*\x,-7);
  \draw[black!20] (2*\x,-8) -- (2*\x,-9);
}
...
\node [font=\tiny,anchor=base west] at (8,8.2)  {\textsf{128}};
\node [font=\tiny,anchor=base east] at (16,8.2)  {\textsf{0}};
\node [font=\tiny,anchor=base west] at (0,-0.8)  {\textsf{1024}};
\node [font=\tiny,anchor=base east] at (16,-0.8)  {\textsf{0}};
