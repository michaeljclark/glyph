...
\hline
\multicolumn{3}{|l|}{\textbf{address and domain registers}} \\
\hline
...
information for floating-point operations. the \texttt{I, Z, U, O,}
and \texttt{P} fields contain accrued floating-point exceptions.
...
contains the current floating-point rounding mode.
this register is visible if \texttt{syscontrol.F} is enabled.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
\subsection{address and domain registers}
the section describes the user-level address and domain registers.
...
this register is visible if \texttt{syscontrol.D} is enabled.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\node [font=\small,anchor=base] at (16.25,0.25)  {\emph{topological address}};
...
when \texttt{control.C} is set, the \emph{capability permission registers}
...
when \texttt{control.C} is set, privileged system registers and
...
permissions. when \texttt{control.C} is set, memory accesses and
...
\hline
\texttt{0x900...0x90f} & domaddr$[n]$   & domain address & \\
\texttt{0xa00...0xa0f} & domptr$[n]$    & domain page table root & \\
\texttt{0xb00...0xb0f} & domthpc$[n]$   & domain trap handler program counter & \\
\texttt{0xc00...0xc0f} & domthib$[n]$   & domain trap handler immediate base & \\
\hline
\end{tabularx}
...
enable the domain extension, control valid domain transitions, and
provide access to last and current domain number and control the
next domain number.
...
this section describes the \emph{domain routing registers} that control routing
for context switching of system traps, domain traps, capability traps and
machine traps.
...
are routed to. system faults for the active domain will context switch
to the domain in this register when the domain extension is enabled in
\texttt{domcontrol.V}.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
are routed to. access to domain registers or instructions for the active
domain will context switch to the domain in this register when the domain
extension is enabled in \texttt{domcontrol.V}.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
are routed to. access to capability registers for the active domain will
context switch to the domain in this register when the domain and
capability extension are both enabled in \texttt{domcontrol.V} and
\texttt{ccontrol.C}. this register is present if both the domain and
capability extensions are present.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
are routed to. access to machine registers for the active domain will
context switch to the domain in this register when the domain and
machine extensions are both enabled in \texttt{domcontrol.V} and
\texttt{scontrol.M}. this register is present if both the domain and
machine extensions are present.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
page translation and trap entry functions for domain context switching.

\subsubsection{domain address (\texttt{domaddr}$[\mathit{ndom}]$)}
\texttt{domaddr}$[\mathit{ndom}]$ are \textbf{\emph{ndom}} read-write
registers containing an array of the domain addresses associated with
each domain. when the domain extension is present and enabled in
\texttt{domcontrol.V} then domain fault context switches will cause the
system to copy the value for the selected default domain into \texttt{sdomain}
for virtual interrupt delivery.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
registers containing an array of the page table root physical addresses
for address translation in each domain. when the domain extension is present
and is enabled in \texttt{domcontrol.V} then domain fault context switches
will cause the system to copy the value for the selected default domain
into \texttt{sptr} for address translation.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
relation to the page table root for the domain. when the domain extension is
present and is enabled in \texttt{domcontrol.V} then domain fault context
switches will cause the system to copy the value for the selected default
domain into \texttt{thpc} and set the \emph{program counter} register.
...
present and is enabled in \texttt{domcontrol.V} then domain fault context
switches will cause the system to copy the value for the selected default
domain into \texttt{thib} and set the \emph{immediate base} register.
...
\draw [black!95] (31,0)    rectangle (32,1);
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\small,anchor=base] at (31.5,0.25)  {\texttt{P}};
\draw [black!95] (30,0)    rectangle (31,1);
\node [font=\tiny,anchor=base] at (30.5,1.25)  {\textsf{1}};
\node [font=\small,anchor=base] at (30.5,0.25)  {\texttt{T}};
\end{tikzpicture}
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & P   & physical permissions check enable \\
\verb|1 << 1| & T   & translation lookaside function enable \\
\hline
\end{tabular}
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & P   & physical permissions check feature \\
\verb|1 << 1| & T   & translation lookaside function feature \\
\hline
\end{tabular}
