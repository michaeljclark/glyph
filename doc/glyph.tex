...
\texttt{0x28} & smessage  & system target message \\
\texttt{0x29} & stimer    & system deadline timer \\
\texttt{0x2a} & sie       & system interrupt enable \\
\texttt{0x2b} & sip       & system interrupt pending \\
\hline
...
target address of a thread to send a virtual interrupt.
all-zeros is the \emph{boot service processor}.
all-ones is the \emph{broadcast address}.
zero in the \emph{A} field is a \emph{topological address}.
one in the \emph{A} field is a \emph{domain address}.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
