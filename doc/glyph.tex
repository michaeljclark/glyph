...
instruction memory is a memory region dedicated to instructions. it is
organized as a word-addressable store like main-memory, but is read-only
and restricted to instructions.
\\
\par\noindent
instruction memory is addressed using \emph{pc-relative} addresses in
instruction immediate values or indirectly in constants that are accessed
using \emph{ib-relative} addresses. section~\ref{sec:constant-stream}
provides details on \emph{ib-relative} addresses.
\\
\par\noindent
...
them together to compose variable-length instructions.
...
\foreach \x in {0,...,1} {
  ...
  \node [font=\small,anchor=base] at (4,\x+0.25)  {\textsf{instrucion \y--\z}};
  \fill [black!5] (8,\x)    rectangle (24,\x+1);
  \foreach \y in {0,...,3} {
...
\node [font=\small,anchor=base] at (4,0.25)  {\textsf{instrucion 4--7}};
\fill [black!5] (8,0)    rectangle (24,1);
\draw [black!95] (8,0)    rectangle (12,1);
\draw [black!95] (12,0)    rectangle (24,1);
\draw [black!95] (20,0)    rectangle (24,1);

\fill [black!5] (0,1)    rectangle (8,2);
\draw [black!95] (0,1)    rectangle (8,2);
\node [font=\small,anchor=base] at (4,1.25)  {\textsf{instrucion 0--3}};
\fill [black!5] (8,1)    rectangle (24,2);
\draw [black!95] (8,1)    rectangle (16,2);
\draw [black!95] (16,1)    rectangle (24,2);
...
constant memory is a memory region dedicated to constant. it is
organized as a word-addressable store like main-memory, but is read-only
and restricted to constants.
\\
\par\noindent
...
\par\noindent
instruction blocks are aligned memory blocks addressed by the \emph{program counter}.
instruction memory is addressed using \emph{pc-relative} addresses in
instruction immediate values or indirectly in constants that are accessed
using \emph{ib-relative} addresses within register slots.
\\
\par\noindent
immediate blocks are aligned memory blocks addressed by the \emph{immediate
base} register. constant memory is addressed using \emph{ib-relative} addresses
within register slots, which are typed, scaled and aligned addresses computed
relative to an \emph{immediate base} register.
\\
\par\noindent
...
\\
\par\noindent
the instruction forms use bonded register slots for immediate operands and
