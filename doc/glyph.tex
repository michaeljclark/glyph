...
\emph{system page table root} register.
...
\hline
\textbf{Levels} &
\textbf{PN\tablefootnote{Page Number} bits} &
\textbf{PO\tablefootnote{Page Offset} bits} &
\textbf{VA\tablefootnote{Virtual Address} bits} &
\textbf{PA\tablefootnote{Physical Address} bits} &
\textbf{Entries} & \textbf{Page sizes} \\
\hline
4 & 9 & 12 & 48 & 64 & 512 & 4KiB, 2Mib, 1GiB \\
\hline
\end{tabular}
...
\par\noindent
page table entries are grouped into arrays within index pages and are
selected by an index derived from a portion of the virtual address. each
entry contains a physical address along with several permission and
metadata bits. the physical address either points to the next page table
...
to the next-level index page, otherwise they are \emph{leaf} entries with
a final translation. entries with the \emph{write} bit set must have the
\emph{read} bit set. the \emph{color} field is reserved for system software
but may be interpreted by a future capabilities system.
...
\node [font=\tiny,anchor=base] at (25.5,1.25)  {\textsf{12}};
\node [font=\tiny,anchor=base] at (0.5,1.25)  {\textsf{63}};
\node [font=\small,anchor=base] at (13.0,0.25)  {\emph{physical page number}};
\end{tikzpicture}
...
\begin{algorithmic}[1]
 ...
 \Function{translate}{$op,va$}
  ...
  \Comment{fetch system page table root pointer}
  \For{$\mathit{level} \gets 1$ to $\mathit{num\_levels}$}
      ...
      \State $\mathit{pte} \gets \Call{load}{(\mathit{ppn}
        << \mathit{po\_bits}) + \mathit{vpn} \times \mathit{sizeof(PTE)}}$
      ...
      \Comment{no read, write or exec is a pointer}
        ...
        \Comment{set pointer to next level}
      ...
      \ElsIf {$\mathit{pte.ppn} \land ((1 << (\mathit{shift} -
        ...
        \State \textbf{raise} \textbf{fault}
      \EndIf
      \State \Return $(\mathit{pte.ppn} << \mathit{po\_bits}) + (\mathit{va} \land
