...
\texttt{0x25} & starget   & system target address \\
\texttt{0x26} & smessage  & system message interrupt \\
\texttt{0x27} & stimer    & system deadline timer \\
...
whether user pages can be executed by the supervisor. the \texttt{P}
field controls whether page table pages need physical maps and
translate permissions. if enabled, then physical addresses must be
self-mapped with the \texttt{T} permission set on entries for pages
that contain page tables.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (31,0)    rectangle (32,1);
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & A   & supervisor access to user pages disabled \\
\verb|1 << 1| & E   & supervisor execute of user pages disabled \\
...
\texttt{sfeature} is a read-only register containing system features.
the \texttt{C} field indicates the capabilities feature is present.
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & C   & capabilities check feature \\
\verb|1 << 0| & P   & physical permissions feature \\
\hline
\end{tabular}
...
\Function{find\_page}{$\mathit{type},\mathit{level},\mathit{ppn},\mathit{ta}$}
  ...
  \State $\mathit{sm} \gets
    \mathit{PTE\_R} \lor \mathit{PTE\_W} \lor \mathit{PTE\_X}$
...
\Function{translate}{$\mathit{op}, \mathit{va}$}
  ...
  \State $\mathit{caps} \gets \mathit{colorperms[pte.color]}$
...
includes several control and status registers for permissions and
capabilities enabled by \emph{color} bits in page table entries.
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\multicolumn{3}{|l|}{\textbf{capability registers}} \\ \hline
\texttt{0x40} & colorperms     & color permissions \\
\texttt{0x41} & colorcaps      & color capabilities \\
\multicolumn{3}{|l|}{\textbf{capability matrix}} \\ \hline
\texttt{0x50} & colormatrix0   & color matrix for color 0 \\
\dots & \dots & \dots \\
\texttt{0x5f} & colormatrix15  & color matrix for color 15 \\
\hline
\end{tabular}
...
\subsubsection{color permissions (\texttt{colorperms})}
\texttt{colorperms} is a read-write register containing an array
of 16 elements holding 4 page table permission mask bits (negated).
when set these bits mask page table permission bits based on the
page table entry \emph{color} field. setting bits masks permissions.
the default of zero as unmasked is to provide full access by
default for non capability aware system software.

\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\foreach \x in {1,...,15} {
\draw [black!95] (30-\x*2.0,0)    rectangle (32-\x*2,1);
}
\node [font=\tiny,anchor=base] at (1,0.3)  {\emph{perm15}};
\node [font=\tiny,anchor=base] at (0.5,1.25)  {\textsf{63}};
\node [font=\tiny,anchor=base] at (1.5,1.25)  {\textsf{60}};
\node [font=\tiny,anchor=base] at (31,0.3)  {\emph{perm0}};
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (30.5,1.25)  {\textsf{3}};
\end{tikzpicture}

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.5]
\draw [black!95] (0,0)    rectangle (2,1);
\draw [black!95] (1.5,0)    rectangle (2,1);
\node [font=\tiny,anchor=base] at (1.75,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (1.75,0.3)  {\texttt{R}};
\draw [black!95] (1.0,0)    rectangle (1.5,1);
\node [font=\tiny,anchor=base] at (1.25,1.25)  {\textsf{1}};
\node [font=\tiny,anchor=base] at (1.25,0.3)  {\texttt{W}};
\draw [black!95] (0.5,0)    rectangle (1.0,1);
\node [font=\tiny,anchor=base] at (0.75,1.25)  {\textsf{2}};
\node [font=\tiny,anchor=base] at (0.75,0.3)  {\texttt{X}};
\draw [black!95] (0.0,0)    rectangle (0.5,1);
\node [font=\tiny,anchor=base] at (0.25,1.25)  {\textsf{3}};
\node [font=\tiny,anchor=base] at (0.25,0.3)  {\texttt{T}};
\end{tikzpicture}
\caption{color permission structure.}
\label{fig:color-permission-structure}
\end{figure}
...
\hline
\textbf{no.} & \textbf{mnem} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & R   & \textasciitilde read       & read permission for \emph{color-x} \\
\verb|1 << 1| & W   & \textasciitilde write      & write permission for \emph{color-x} \\
\verb|1 << 2| & X   & \textasciitilde execute    & execute permission for \emph{color-x} \\
\verb|1 << 3| & P   & \textasciitilde physical   & physical permission for \emph{color-x} \\
\hline
\end{tabular}
\caption{color permission descriptions.}
\label{tab:color-permission-descriptions}
\end{table}

\newpage
\subsubsection{color capabilities (\texttt{colorcaps})}
\texttt{colorcaps} is a read-write register containing an array of 16
elements holding 4 capability bits (negated). when set these bits mask
access to the \emph{debug, system, capabilites,} and \emph{hypervisor}
sets of registers and instructions from program text pages based on
the page table entry \emph{color} field. setting bits masks capabilities.
the default of zero as unmasked is to provide full access by default for
non capability aware system software.
...
\node [font=\tiny,anchor=base] at (1,0.3)  {\emph{set15}};
\node [font=\tiny,anchor=base] at (0.5,1.25)  {\textsf{63}};
\node [font=\tiny,anchor=base] at (1.5,1.25)  {\textsf{60}};
\node [font=\tiny,anchor=base] at (31,0.3)  {\emph{set0}};
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (30.5,1.25)  {\textsf{3}};
\end{tikzpicture}
...
\draw [black!95] (1.5,0)    rectangle (2,1);
\node [font=\tiny,anchor=base] at (1.75,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (1.75,0.3)  {\texttt{D}};
\draw [black!95] (1.0,0)    rectangle (1.5,1);
\node [font=\tiny,anchor=base] at (1.25,1.25)  {\textsf{1}};
\node [font=\tiny,anchor=base] at (1.25,0.3)  {\texttt{S}};
\draw [black!95] (0.5,0)    rectangle (1.0,1);
\node [font=\tiny,anchor=base] at (0.75,1.25)  {\textsf{2}};
\node [font=\tiny,anchor=base] at (0.75,0.3)  {\texttt{C}};
\draw [black!95] (0.0,0)    rectangle (0.5,1);
\node [font=\tiny,anchor=base] at (0.25,1.25)  {\textsf{3}};
\node [font=\tiny,anchor=base] at (0.25,0.3)  {\texttt{H}};
\end{tikzpicture}
\caption{color capabilities structure.}
\label{fig:color-capabilities-structure}
\end{figure}
...
\textbf{no.} & \textbf{mnem} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & D   & \textasciitilde debug      & debug access from \emph{color-x} \\
\verb|1 << 1| & S   & \textasciitilde system     & system access from \emph{color-x} \\
\verb|1 << 2| & C   & \textasciitilde capability & capability access from \emph{color-x} \\
\verb|1 << 3| & H   & \textasciitilde hypervisor & hypervisor access from \emph{color-x} \\
\hline
\end{tabular}
\caption{color permission descriptions.}
\label{tab:color-permission-descriptions}
...
\subsubsection{color matrix (\texttt{colormatrix0\dots colormatrix15})}
\texttt{colormatrix0\dots colormatrix15} are read-write registers
containing arrays of 16 elements holding 4 (negated) permission bits.
when set these bits mask page table permission bits or branch execute
permissions from executable pages based on the source page table entry
\emph{color-x}, with source color in the register name, for memory
access or branch to target pages with page table entry \emph{color-y},
with target color in the array index within each register.
...
\node [font=\tiny,anchor=base] at (0.25,1.25)  {\textsf{3}};
\end{tikzpicture}
\caption{color matrix structure.}
\label{fig:color-matrix-structure}
\end{figure}
...
\textbf{no.} & \textbf{mnem} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & R   & \textasciitilde read   &
load permission from \emph{color-x} to \emph{color-y} \\
\verb|1 << 1| & W   & \textasciitilde write  &
store permission from \emph{color-x} to \emph{color-y} \\
\verb|1 << 2| & X   & \textasciitilde execute &
branch permission from \emph{color-x} to \emph{color-y} \\
\hline
\end{tabular}
\caption{color matrix desciptions.}
\label{tab:color-matrix-desciptions}
\end{table}
