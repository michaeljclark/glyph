...
\\
\par\noindent
immediate blocks are 64-byte aligned blocks addressed by the \emph{immediate
base register} which contain eight 64-bit constants. immediate blocks can be
navigated by branching the constant stream independently using the
\emph{immediate-block-jump} instruction. return from procedure restores
\emph{(pc,ib)} using a link register containing a packed \emph{i32x2} relative
address vector to support backward compatibility with a single link register.
\\
\par\noindent
...
\begin{itemize}[itemsep=0pt]
\item 16-bit instruction packet can access \emph{$8 \times 64\text{-}bit$}
      registers with up to 3 operands.
\item 32-bit instruction packet can access \emph{$64 \times 64\text{-}bit$}
      registers with up to 3 operands.
\item 64-bit instruction packet can access \emph{$64 \times 64\text{-}bit$}
...
\begin{itemize}[itemsep=0pt]
\item \emph{$64\text{-}bit$} program counter register (aligned to 2 bytes).
\item \emph{$64\text{-}bit$} immediate base register (aligned to 64 bytes).
\item \emph{$8 \times 64\text{-}bit$} general purpose registers.
\item \emph{$1 \times 1\text{-}bit$} predicate register.
\end{itemize}
