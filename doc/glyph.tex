...
address translation. the \texttt{S} field is read-write and controls whether
user pages can be executed by the supervisor. the \texttt{A} field is
read-write and controls whether the user pages and be read or written by
the supervisor.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\draw [black!95] (30,0)    rectangle (31,1);
\node [font=\tiny,anchor=base] at (30.5,1.25)  {\textsf{1}};
\node [font=\small,anchor=base] at (30.5,0.25)  {\texttt{A}};
\end{tikzpicture}
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 1| & S   & supervisor execute user pages \\
...
\emph{pointer} field contains the physical address of the page table
root structure. the page table root pointer structure:
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\node [font=\small,anchor=base] at (15,0.25)  {\emph{pointer}};
...
\texttt{starget} is a read-write register containing the target system-wide
address for a hardware thread that is the target of a virtual interrupt.
a target address of all-zeros is the boot service processor. a target
address of all-ones is the broadcast address for all threads.

\subsubsection{system message (\texttt{smessage})}
\texttt{smessage} is a write-only register that triggers an inter-processor
interrupt causing a virtual interrupt to be queued to the hardware thread
set in \texttt{starget}. if interrupts are not enabled on the target hardware
thread, the virtual interrupt is delivered when next enabled.
...
the section describes the page table structure for memory address
translation pointed to by the \emph{system page table root} register.
page tables are 4KiB pages containing 512 entries using the following
format for recursive address translation for 4KiB pages.
\\
\par\noindent
the 8-byte page table entry structure:
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\\
\par\noindent
the 64-bit lookup virtual address structure:
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (26,0)   rectangle (32,1);
...
\\
\par\noindent
the 64-bit translated physical address structure:
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (26,0)   rectangle (32,1);
