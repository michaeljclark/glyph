...
includes privileged registers for permissions and capabilities
...
privileged capability registers are available when \texttt{scontrol.C}
is enabled and the associated executable page table colors are configured
in \emph{colorcap}.
\\
\par\noindent
...
\texttt{0x50...0x5f} & colormatread$[n]$   & color matrix read permissions \\
\texttt{0x60...0x6f} & colormatwrite$[n]$   & color matrix write permissions \\
\texttt{0x70...0x7f} & colormatexec$[n]$   & color matrix exec permissions \\
\hline
\end{tabularx}
...
\subsubsection{color matrix read permissions (\texttt{colormatread}$[\mathit{ncols}]$)}
\texttt{colormatread}$[\mathit{ncols}]$ are \textbf{\emph{ncol}} read-write
...
\subsubsection{color matrix write permissions (\texttt{colormatwrite}$[\mathit{ncols}]$)}
\texttt{colormatwrite}$[\mathit{ncols}]$ are \textbf{\emph{ncol}} read-write
...
\subsubsection{color matrix exec permissions (\texttt{colormatexec}$[\mathit{ncols}]$)}
\texttt{colormatexec}$[\mathit{ncols}]$ are \textbf{\emph{ncol}} read-write
