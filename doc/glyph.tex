...
the architecture provides several user-legel status and control registers
for floating-point status and control, clock time, clock frequency,
thread address and thread domain.
\\
\par\noindent
...
\\
\par\noindent
privileged system status and control registers can be read and written
...
from executable pages with a supervisor or physical address spaces.
...
contains the address space identifier for this thread.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\hline
\texttt{0x1000...0x100f} & domsuper$[n]$  & domain supervisor default domain \\
...
register is visible to the supervisor.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
for context switching of supervisor traps, domain traps and capability traps.

\subsubsection{domain supervisor default domain vector (\texttt{domsuper}$[\mathit{ndom}]$)}
\texttt{domsuper}$[\mathit{ndom}]$ are \textbf{\emph{ndom}} read-write
registers containing the domain number of the domain that supervisor traps
are routed to. supervisor faults for the active domain will context switch
...
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (24.5,1.25)  {\textsf{ndom}};
\node [font=\small,anchor=base] at (28.0,0.25)  {\emph{supervisor domain}};
\end{tikzpicture}
...
for virtual interrupt delivery to the domain.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
registers containing an array of the page table root for address translation
in each domain. when the domain feature is present and is enabled in
\texttt{scontrol.V} then domain fault context switches will cause the
system to copy the value for the selected default domain into \texttt{sptr}
for address translation in the domain.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
canonical address form for user and supervisor virtual addresses as
...
prefix to allow them to contain user and supervisor virtual addresses
