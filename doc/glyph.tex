...
the architecture provides several status and control registers for
floating-point status and control, time, trap handling, address translation,
timers, inter-thread interrupts, and debug.
\\
\par\noindent
user-level and privileged system status and control registers can be read
and written using the \texttt{sysread}, \texttt{syswrite}, \texttt{sysset},
and \texttt{sysclear} instructions. user-level and privileged trap status
and contol registers, system status and control registers and debug registers
are available in contexts where their capability set is enabled.
\\
\par\noindent
the architectural status and control registers are divided into
four categories:
\\
\par\noindent
\begin{table}[H]
\centering
\footnotesize
\begin{tabularx}{120mm}{|l|X|}
\hline
\textbf{name} & \textbf{description} \\
\hline
user   & floating-point control, status, clock time and frequency. \\
trap   & trap control, status, context, handlers and vectors. \\
system & system control, status, address translation and interrupts. \\
debug  & preformance counters and debug monitoring. \\
\hline
\end{tabularx}
\caption{system register categories}
\label{tab:system-register-categories}
\end{table}
...
\newpage
\subsubsection{privileged trap registers}
the following table lists the privileged trap registers:
...
\hline
\end{tabularx}
\caption{privileged trap registers}
\label{tab:privileged-trap-registers}
\end{table}

\subsubsection{privileged system registers}
the following table lists the privileged system registers:
...
\hline
\end{tabularx}
\caption{privileged system registers}
\label{tab:privileged-system-registers}
\end{table}

\subsubsection{privileged debug registers}
the following table lists the privileged debug registers:
...
\hline
\end{tabularx}
\caption{privileged debug registers}
\label{tab:privileged-debug-registers}
\end{table}
...
the \texttt{A} field indicates the user address space access disable feature is enabled.
the \texttt{E} field indicates the user address space execute disable feature is enabled.
the \texttt{P} field indicates the physical permissions feature is enabled.
the \texttt{C} field indicates the capabilities check feature is enabled.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\draw [black!95] (29,0)    rectangle (30,1);
\node [font=\tiny,anchor=base] at (29.5,1.25)  {\textsf{2}};
\node [font=\small,anchor=base] at (29.5,0.25)  {\texttt{P}};
\draw [black!95] (28,0)    rectangle (29,1);
\node [font=\tiny,anchor=base] at (28.5,1.25)  {\textsf{3}};
\node [font=\small,anchor=base] at (28.5,0.25)  {\texttt{C}};
\end{tikzpicture}
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & A   & user address space access disable feature enabled \\
\verb|1 << 1| & E   & user address space execute disable feature enabled \\
\verb|1 << 2| & P   & physical permissions feature enabled \\
\verb|1 << 3| & C   & capabilities check feature enabled \\
\hline
\end{tabular}
...
the \texttt{A} field indicates the user address space access disable feature is present.
the \texttt{E} field indicates the user address space execute disable feature is present.
the \texttt{P} field indicates the physical permissions feature is present.
the \texttt{C} field indicates the capabilities check feature is present.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\verb|1 << 0| & A   & user address space access disable feature \\
\verb|1 << 1| & E   & user address space execute disable feature \\
\verb|1 << 2| & P   & physical permissions feature \\
\verb|1 << 3| & C   & capabilities check feature \\
\hline
\end{tabular}
...
\node [font=\small,anchor=base] at (16.25,0.25)  {\emph{system topological address}};
...
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (1.0,1.25)  {\textsf{62}};
\node [font=\small,anchor=base] at (16.25,0.25)  {\emph{system domain address}};
\node [font=\tiny,anchor=base] at (0.25,1.25)  {\textsf{63}};
...
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (1.0,1.25)  {\textsf{62}};
\node [font=\small,anchor=base] at (16.25,0.25)  {\emph{system target address}};
\node [font=\tiny,anchor=base] at (0.25,1.25)  {\textsf{63}};
...
includes several control registers for permissions and capabilities
enabled by \emph{color} bits in page table entries.
\\
\par\noindent
the following tables list the capability system parameters and registers:
...
\texttt{0x44} & colorcap       & color capability capabilities \\
...
that are used to check loads from pages with a matching color.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
that are used to check stores to pages with a matching color.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (24,0)    rectangle (32,1);
...
of \textbf{\emph{ncol}} bits containing exec permissions for \emph{color-x}
that are used to check execution of pages with a matching color.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (24,0)    rectangle (32,1);
...
from executable pages with a matching color.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (24,0)    rectangle (32,1);
...
executable pages with a matching color.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (24,0)    rectangle (32,1);
...
permissions that are used to check loads from executable pages with matching
register source \emph{color-x} from addresses in pages with matching
index target \emph{color-y}.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (24,0)    rectangle (32,1);
...
permissions that are used to check stores from executable pages with matching
register number \emph{color-x} to addresses in pages with matching
index target \emph{color-y}.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
...
permissions that are used to check branches from executable pages with
matching register source \emph{color-x} to executable pages with
matching index target \emph{color-y}.
\\
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (24,0)    rectangle (32,1);
