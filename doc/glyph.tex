...
glyph achieves this by packing together two 32-bit relative \emph{(pc,ib)}
displacements in an \emph{i32x2} vector.
\\
\par\noindent
...
instructions.
\item 16-bit compressed instruction packets can access 8x64-bit registers.
\item \emph{(pc,ib)} is a special program counter and immediate base
register address vector.
\item link register contains packed \emph{i32x2} relative address vector to
function entry.
\item \texttt{ibl} \emph{immediate-block-link} adds a relative address to
the immediate base register.
\item \texttt{lib} \emph{load-immediate-block} uses unsigned displacement
to access constants.
...
\node [font=\small,above,scale=0.9,anchor=south] at (8,2)  {\textbf{16-bit instruction packet}};
...
\node [font=\small,above,scale=0.9,anchor=south] at (8,-10)  {\textbf{64-bit immediate data}};
...
\\
\par\noindent
immediate blocks can navigated by branching the constant stream independently
with the \emph{ib-link} instruction. the link register contains packed
\emph{i32x2} relative address vector to support backward compatibility
with a single link register.
\\
\par\noindent
...
\begin{itemize}[itemsep=0pt]
\item 16-bit instruction packet can access \emph{8 * 64-bit}
      registers with up to 3 operands.
\item 32-bit instruction packet can access \emph{64 * 64-bit}
      registers with up to 3 operands.
\item 64-bit instruction packet can access \emph{64 * 64-bit}
      registers with up to 6 operands.
\end{itemize}
...
\node [font=\tiny,anchor=base] at (15.5,11.2)  {$\textbf{64}$};
...
\end{scope}
\end{tikzpicture}
\caption{64-bit register state accessible by 16-bit instruction packet.}
...
\needspace{8\baselineskip}
\subsection{ibl}
\insrcimmsix{rc}{uimm}{00011}{00}{\textbf{ibl} rc, uimm6}
\\
the \emph{ibl} or \emph{immediate-block-link} instruction saves
the old immediate base register to the desination register and
adds a 64-bit relative address stored in constant memory at
address \emph{ib + simm6 * 8}, to the immediate base register.
\begin{Verbatim}[baselinestretch=1.0,fontsize=\small]
    rc = ib
    ib = (i64)[ib + uimm6 * 8]
\end{Verbatim}
...
\\
the \emph{jtlib} or \emph{jump-and-link-immediate-block} instruction
loads a 64-bit constant addressed by \emph{ib + uimm6 * 8} containing a
...
\needspace{8\baselineskip}
\subsection{sll.i64}
\insrcrbrc{rc}{rb}{ra}{11011}{00}{\textbf{sll.i64} rc, rb, ra6}
\\
...
\\
the \emph{add} instruction adds the \emph{ra} register to
the \emph{rb} register and saves the result in the \emph{rc} register.
\begin{Verbatim}[baselinestretch=1.0,fontsize=\small]
    r[rc] = r[rb] + r[ra]
\end{Verbatim}
...
\needspace{8\baselineskip}
\subsection{nop}
\insnimmnine{uimm}{11101}{00}{\textbf{nop} uimm9}
\\
the \emph{nop} or \emph{no-operation} instruction does nothing.

\needspace{8\baselineskip}
\subsection{ud1}
\insnimmnine{uimm}{11110}{00}{\textbf{ud1} uimm9}
\\
the \emph{ud1} or \emph{undefined-1} instruction causes an
illegal instruction trap.

\needspace{8\baselineskip}
\subsection{ud2}
\insnimmnine{uimm}{11111}{00}{\textbf{ud2} uimm9}
\\
the \emph{ud2} or \emph{undefined-2} instruction causes an
illegal instruction trap.
...
\ainsrcimmsix{3}{rc}{uimm}{00011}{00}{\textbf{ibl} rc, uimm6}
...
\ainsrcrbrc{27}{rc}{rb}{ra}{11011}{00}{\textbf{sll.i64} rc, rb, ra6}
\ainsrcrbrc{28}{rc}{rb}{ra}{11100}{00}{\textbf{add.i64} rc, rb, ra}
\ainsnimmnine{29}{uimm}{11101}{00}{\textbf{nop} uimm9}
\ainsnimmnine{30}{uimm}{11110}{00}{\textbf{ud1} uimm9}
\ainsnimmnine{31}{uimm}{11111}{00}{\textbf{ud2} uimm9}
