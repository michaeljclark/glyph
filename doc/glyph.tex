...
\emph{strongly-ordered} operations with writes must act as
implicit write ordering barriers for relaxed memory operations.
this means any instruction with \emph{strongly-ordered} writes must
act as an implicit write barrier: relaxed writes may not be reordered
with respect to a \emph{strongly-ordered} or \emph{synchronized} memory
write, and relaxed reads may not be reordered before such a write:
\emph{write–write} and \emph{write–read} ordering.
\\
\par\noindent
translation to \emph{weakly-ordered} memory systems with \emph{acquire-release}
ordering requires implicit barriers around atomic write operations because
