...
\item the domain number for \emph{domain faults} when the domain extension
is enabled and \emph{capability faults} when both capabilities and domain extensions are enabled.
\end{itemize}
...
\hline
\textbf{\emph{ncol}}      & the number of architectural colors \\
\hline
\end{tabularx}
...
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (24.5,1.25)  {\textsf{ncol}};
...
domains and capabities extensions to be present and enabled.
\\
\par\noindent
...
\textbf{name} & \textbf{description} \\
\hline
\textbf{\emph{ndom}}      & the number of architectural domains \\
\hline
\end{tabularx}
...
\textbf{no.} & \textbf{name} & \textbf{description} \\
\hline
\multicolumn{3}{|l|}{\textbf{domain status registers}} \\
\hline
\texttt{0x50} & dom      & domain number \\
\hline
\multicolumn{3}{|l|}{\textbf{domain routing registers}} \\
\hline
\texttt{0x1000...0x100f} & domsys$[n]$    & domain system default domain vector \\
\texttt{0x2000...0x200f} & domdom$[n]$    & domain domain default domain vector \\
\texttt{0x3000...0x300f} & domcap$[n]$    & domain capability default domain vector \\
\hline
\multicolumn{3}{|l|}{\textbf{domain context registers}} \\
\hline
\texttt{0x4000...0x400f} & domaddr$[n]$   & domain address vector \\
\texttt{0x5000...0x500f} & domptr$[n]$    & domain page table root vector \\
\texttt{0x6000...0x600f} & domthpc$[n]$   & domain trap handler program counter vector  \\
\texttt{0x7000...0x700f} & domthib$[n]$   & domain trap handler immediate base vector \\
\hline
\end{tabularx}
...
\newpage
\subsection{domain status registers}

this section describes the \emph{domain status registers}.

\subsubsection{domain number (\texttt{dom})}
\texttt{dom} is a read-write register containing the number of the
currently running domain. it is initialized to zero. when the domain
extension feature is present and enabled in \texttt{scontrol.V} this
register is visible to the system.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
\draw [black!95] (0,0)    rectangle (32,1);
\draw [black!95] (24,0)    rectangle (32,1);
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (24.5,1.25)  {\textsf{ndom}};
\node [font=\small,anchor=base] at (28.0,0.25)  {\emph{domain number}};
\end{tikzpicture}
...
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (24.5,1.25)  {\textsf{ndom}};
\node [font=\small,anchor=base] at (28.0,0.25)  {\emph{system domain}};
\end{tikzpicture}
...
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (24.5,1.25)  {\textsf{ndom}};
\node [font=\small,anchor=base] at (28.0,0.25)  {\emph{domain domain}};
\end{tikzpicture}
...
capabilities extensions are both enabled in \texttt{scontrol.V} and
\texttt{scontrol.C}.
\\
\par\noindent
\begin{tikzpicture}[scale=0.45]
...
\node [font=\tiny,anchor=base] at (31.5,1.25)  {\textsf{0}};
\node [font=\tiny,anchor=base] at (24.5,1.25)  {\textsf{ndom}};
\node [font=\small,anchor=base] at (28.0,0.25)  {\emph{capability domain}};
\end{tikzpicture}
